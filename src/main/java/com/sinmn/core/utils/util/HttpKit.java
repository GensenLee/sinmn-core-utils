 ...
 * 
 * @ClassName: HttpKit
 * @Description: 网络请求（ 微信为https请求）
 * @date 2015年4月17日 下午9:25:03
 *
 */
public class HttpKit {
	...
	public static String post(String url, String params, Map<String, String> headers) {
		String log = "post(" + url + "," + params + "," + headers + ")";
		StringBuffer bufferRes = null;
		try {
			HttpURLConnection http = null;
			if (isHttps(url)) {
			...
			e.printStackTrace();
			logger.error(log, e);
			String result = postRetry(url, params, headers);
			if (result == null) {
				result = postRetry(url, params, headers);
			}
			return result;
		}
	}

	public static String postRetry(String url, String params, Map<String, String> headers) {
		String log = "post(" + url + "," + params + "," + headers + ")";
		StringBuffer bufferRes = null;
		try {
			HttpURLConnection http = null;
       ...
       try { 
           ...
           try { 
               ...
               if (resEntity != null) { 
                   ...
                   ApiResult apiResult = FastJsonUtils.getBean(responseEntityStr, ApiResult.class);
                   result = apiResult.getObject().toString();
               } 
